    ...
    <el-menu :default-active="defaultActive" mode="horizontal">
      <template v-for="(item, index) in items">
        <el-menu-item :index="String(index)">
          <router-link :to="'/table/table' + (index + 1)">
            {{ item }}
          </router-link>
        </el-menu-item>
      </template>
    </el-menu>
    <div style="width:1200px;margin:10px;border:2px solid blue;">
      <router-view />
    </div>
...
export default {
  mounted() {
    this.defaultActive = String(Number(this.$route.fullPath.slice(12)) - 1);
  },
  data() {
    return {
      defaultActive: "0",
      ...
      datas6: {
        title: "Table-column Scoped Slot",
        head: ["name", "说明"],
        body: [
